...
export default function RecruiterSieve() {
  const [messages, setMessages] = useState<Message[]>([]);
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  const [suggestions, setSuggestions] = useState<string[]>([
    "Walk me through a client submittal",
    "Generate a candidate brief for me",
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Your personal recruiting concierge
          </p>
        </div>
        {messages.length > 0 && (
          <button
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <h2 className="mt-4 text-lg font-semibold text-slate-900">
              Hi! I&apos;m Sieve /siv/, your recruiting concierge.
            </h2>
            <p className="mt-1 max-w-md text-sm text-slate-500">
              I can help with pipeline strategy, client submittals, candidate
              briefs, sourcing tips, and more. Ask me anything!
            </p>
          </div>
        )}
        {messages.map((msg, i) => (
