...
with engine.connect() as c:
    # Find user
    user = c.execute(text(
        "SELECT id, email, role, created_at FROM users WHERE email = 'lakshmi@test.com'"
    )).fetchone()
    if not user:
    ...
    trust = c.execute(text(
        "SELECT * FROM candidate_trust WHERE user_id = :uid"
    ), {"uid": uid}).fetchone()
    if trust:
    ...
    logs = c.execute(text(
        "SELECT * FROM trust_audit_log WHERE user_id = :uid ORDER BY created_at DESC"
    ), {"uid": uid}).fetchall()
    if logs:
