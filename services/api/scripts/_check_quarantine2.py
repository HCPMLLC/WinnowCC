...
with engine.connect() as c:
    ...
    cols = c.execute(text(
        "SELECT column_name FROM information_schema.columns WHERE table_name = 'candidate_trust' ORDER BY ordinal_position"
    )).fetchall()
    print(f"candidate_trust columns: {[r[0] for r in cols]}")

    # Check trust_audit_log columns
    cols2 = c.execute(text(
        "SELECT column_name FROM information_schema.columns WHERE table_name = 'trust_audit_log' ORDER BY ordinal_position"
    )).fetchall()
    print(f"trust_audit_log columns: {[r[0] for r in cols2]}")

    # Query candidate_trust for this user (use correct column)
    trust = c.execute(text(
        "SELECT * FROM candidate_trust WHERE candidate_id = :uid"
    ), {"uid": uid}).fetchone()
    if trust:
    ...
    logs = c.execute(text(
        "SELECT * FROM trust_audit_log WHERE candidate_id = :uid ORDER BY created_at DESC"
    ), {"uid": uid}).fetchall()
    if logs:
    ...
    docs = c.execute(text(
        "SELECT id, filename, file_hash, trust_score, uploaded_at FROM resume_documents WHERE user_id = :uid ORDER BY uploaded_at DESC"
    ), {"uid": uid}).fetchall()
    if docs:
    ...
    profiles = c.execute(text(
        "SELECT id, user_id, profile_version, trust_score, quarantine FROM candidate_profiles WHERE user_id = :uid ORDER BY id DESC"
    ), {"uid": uid}).fetchall()
    if profiles:
        print(f"\nCandidate Profiles ({len(profiles)}):")
        for p in profiles:
            print(f"  id={p[0]} user={p[1]} version={p[2]} trust={p[3]} quarantine={p[4]}")
    else:
