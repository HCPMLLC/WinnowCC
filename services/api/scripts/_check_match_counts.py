...
with engine.connect() as c:
    ...
    rows = c.execute(
        text(
            "SELECT ejc.employer_job_id, ej.title, "
            "COUNT(*) as cnt, "
            "array_agg(ejc.match_score) "
    ...
    print("\n=== Employer Jobs ===")
    jobs = c.execute(
        text("SELECT id, title, status, employer_id FROM employer_jobs ORDER BY id")
    ).fetchall()
    for j in jobs:
    ...
    print("\n=== Employer Profiles ===")
    profiles = c.execute(
        text("SELECT id, user_id, company_name FROM employer_profiles")
    ).fetchall()
    for p in profiles:
