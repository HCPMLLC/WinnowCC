...
def main():
    ...
    # Schedule reconcile stale batches (every 10 minutes)
    _jtype = "reconcile_batches"
    for job in scheduler.get_jobs():
        meta = getattr(job, "meta", {})
        if meta.get("scheduled_job_type") == _jtype:
            scheduler.cancel(job)

    reconcile_job = scheduler.cron(
        cron_string="*/10 * * * *",
        func=(
            "app.services.scheduled_jobs"
            ":scheduled_reconcile_stale_batches"
        ),
        queue_name="default",
        meta={"scheduled_job_type": _jtype},
    )
    logger.info("Reconcile stale batches: %s", reconcile_job.id)

    # Schedule start queued imports (every 2 minutes)
    _jtype = "start_queued_imports"
    for job in scheduler.get_jobs():
        meta = getattr(job, "meta", {})
        if meta.get("scheduled_job_type") == _jtype:
            scheduler.cancel(job)

    qi_job = scheduler.cron(
        cron_string="*/2 * * * *",
        func=(
            "app.services.scheduled_jobs"
            ":scheduled_start_queued_imports"
        ),
        queue_name="default",
        meta={"scheduled_job_type": _jtype},
    )
    logger.info("Start queued imports: %s", qi_job.id)

    # Schedule session cleanup (daily at 3:30am UTC)
    for job in scheduler.get_jobs():
        if (
            hasattr(job, "meta")
            and job.meta.get("scheduled_job_type") == "session_cleanup"
        ):
            scheduler.cancel(job)
            logger.info(f"Cancelled existing session cleanup job: {job.id}")

    session_cleanup_job = scheduler.cron(
        cron_string="30 3 * * *",
        func="app.services.scheduled_jobs:scheduled_cleanup_expired_sessions",
        queue_name="default",
        meta={"scheduled_job_type": "session_cleanup"},
    )
    logger.info(
        f"Scheduled session cleanup registered: {session_cleanup_job.id}"
    )

    # Schedule auth event purge (daily at 4:30am UTC)
    for job in scheduler.get_jobs():
        if (
            hasattr(job, "meta")
            and job.meta.get("scheduled_job_type") == "auth_event_purge"
        ):
            scheduler.cancel(job)
            logger.info(f"Cancelled existing auth event purge job: {job.id}")

    auth_purge_job = scheduler.cron(
        cron_string="30 4 * * *",
        func="app.services.scheduled_jobs:scheduled_purge_old_auth_events",
        queue_name="default",
        meta={"scheduled_job_type": "auth_event_purge"},
    )
    logger.info(
        f"Scheduled auth event purge registered: {auth_purge_job.id}"
    )
